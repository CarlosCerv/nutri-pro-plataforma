...
- **Posible tercer motor de % de grasa corporal, sin evaluar.** `backend/src/services/nutritionCalculator.js` (`calculateBodyFat3Site`/`calculateBodyFat7Site`, usado por `bodyComposition.controller.js` al guardar una medición) implementa las mismas fórmulas Jackson-Pollock que `frontend/src/lib/calculations/bodyFat.js` usa en el cliente. A diferencia de `utils/calculations.ts` (ver "Resuelto"), este sí es el mismo cálculo duplicado en dos lugares — servidor y cliente — y podría divergir si uno de los dos se toca sin el otro. No se ha decidido si vale la pena centralizarlo (por ejemplo, que el cliente llame a un endpoint en vez de recalcular) o si duplicarlo es aceptable porque nunca se toca.
...
- **`seedGeneratedTemplates.js` sigue teniendo 7 combinaciones (de 340) con una porción de un solo alimento generosa pero no absurda** (350-410g de frijol o pan repartido) en los rangos de más calorías — el propio script las señala en su reporte de validación (`npm run seed:generated-templates:dry-run`), no se corrigieron una por una porque no son un error, solo una porción grande a criterio del nutriólogo.
...
- **`/api/calculations/*` archivado** (`f097cdf`) a `backend/src/_archive/`; ya no está montado en `app.js`.
- **Cerrado el tema abierto #3 ("fusionar los dos motores de cálculo del front end").** Al revisarlo a fondo, `utils/calculations.ts` (agregación de macros de comidas, usado solo por `useMealPlanner`) y `src/lib/calculations/` (fórmulas clínicas antropométricas — IMC, TMB, % grasa, IDR — usadas por las páginas) no calculan lo mismo: no son un duplicado que compita por la misma responsabilidad, son dos dominios distintos con bases de consumidores separadas. No había nada que fusionar. Se decidió en su lugar unificar la convención de carpeta: el contenido de `utils/calculations.ts` se movió a `src/lib/calculations/mealPlan.ts` (mismo código, import actualizado en `useMealPlanner.ts` y su test), y `utils/` se eliminó por quedar vacía. Queda como hallazgo nuevo, sin resolver, la posible redundancia entre `backend/src/services/nutritionCalculator.js` y `frontend/src/lib/calculations/bodyFat.js` — ver "Detectada en la última auditoría".
